...
import { FormParamDefinition, Header, QueryParamDefinition, Request, RequestBody, RequestBodyDefinition } from 'postman-collection';
 ...
 * @param {Boolean} trim indicates whether to trim string or not
 */
function parseHeaders(headersArray: Header[], indent: string, trim: boolean) {
  var headerString = '',
  ...
  forEach(headersArray, function (header: Header) {
    if (!header.disabled) {
...
export function convert(request: Request, options: SnippetOptions, callback: Function) {
  ...
  const headers = parseHeaders(request.headers.all(), indent, trim),
