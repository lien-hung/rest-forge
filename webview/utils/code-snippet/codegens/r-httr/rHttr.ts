...
const parseBody = require('./util/parseBody').default.parseBody;
...
/**
  ...
  */
function getSnippetHeader(): string {
  return 'library(httr)\n\n';
}
 ...
 */
function getRequestHeaders(request: Request): Header[] {
 ...
 */
function getSnippetArray(mapToSnippetArray: { key: string; value: string }[], indentation: string, sanitize: boolean): string {
 ...
 */
function groupHeadersSameKey(headers: Header[]): Array<{ key: string; value: string }> {
  ...
  groupedList = Object.keys(grouped).map((key) => ({ key, value: grouped[key].join(', ') }));
  return groupedList;
}
...
function getSnippetHeaders(headers: Header[], indentation: string): string {
  if (headers.length > 0) {
    ...
    const groupedHeaders = groupHeadersSameKey(headers);
    return `headers = ${getSnippetArray(groupedHeaders, indentation, true)}\n\n`;
  }
  return '';
}
 ...
 */
function getEncodeSnippetByMode(mode: string): string {
...
/**
  ...
  */
function getSnippetFromMethod(url: string, hasParams: boolean, hasHeaders: boolean, methodUC: string, mode: string, requestTimeout: number = 0): string {
