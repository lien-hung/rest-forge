...
import { forEach, isFunction } from '../../common/lodash';
...
function makeSnippet(request: Request, indentString: string, options: SnippetOptions): string {
  ...
  if (request.body && request.body.mode === 'formdata') {
    forEach(request.body.formdata?.all() ?? [], function (data) {
  ...
  if (request.body && request.body.mode in request.body) {
    optionsArray.push(
      indentString + parseBody(request.body, indentString, options.trimRequestBody, request.headers.get('Content-Type'))
    );
  }
