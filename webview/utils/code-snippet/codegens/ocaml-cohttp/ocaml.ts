...
function parseRawBody(body: string, mode: string, trim: boolean): string {
  var bodySnippet = `let postData = ref ${sanitize(body, mode, trim)};;\n\n`;
  return bodySnippet;
}
 ...
 * @returns {String} request body in the desired format
 */
function parseURLEncodedBody(body: PropertyList<QueryParam>, mode: string, trim: boolean): string {
  var payload, bodySnippet;
  payload = body.all().reduce(function (accumulator, data) {
    if (!data.disabled) {
 ...
 * @returns {String} request body in the desired format
 */
function parseFormData(body: PropertyList<FormParam>, trim: boolean, indent: string): string {
  var parameters = '[|\n' + body.all().reduce((accumalator, data) => {
    if (!data.disabled) {
      ...
      if ('type' in data && data.type === 'file' && 'src' in data) {
        ...
        accumalator.push(`${indent}[| ("name", "${key}"); ("value", "${value}")` +
          ('contentType' in data ? `; ("contentType", "${data.contentType}")` : '') +
          ' |]');
      }
    }
    return accumalator;
...
function parseBody(body: RequestBody, trim: boolean, indent: string): string {
  if (!isEmpty(body)) {
    switch (body.mode) {
      case 'urlencoded':
        return body.urlencoded ? parseURLEncodedBody(body.urlencoded, body.mode, trim) : '';
      case 'raw':
        return parseRawBody(String(body.raw), body.mode, trim);
      case 'formdata':
        return body.formdata ? parseFormData(body.formdata, trim, indent) : '';
      case 'file':
        return parseFile();
      default:
        return '';
 ...
 */
function parseHeaders(bodyMode: string, headers: HeaderDefinition[], indent: string): string {
  var headerSnippet = '';
  if (!isEmpty(headers)) {
 ...
 */
function getMethodArg(method: string): string {
  var methodArg = '',
    supportedMethods = ['GET', 'POST', 'PUT', 'PATCH', 'DELETE', 'HEAD'],
    flag = false;

  forEach(supportedMethods, (value) => {
    if (value === method) {
      flag = true;
    }
  });
