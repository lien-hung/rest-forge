import { get, isFunction } from '../../common/lodash';
import { parseHeader, parseBody } from './util/parseRequest';
import { addDefaultContentType } from './util/formatRequest';
...
const ALLOWED_METHODS = ['GET', 'POST', 'PUT', 'DELETE', 'HEAD', 'OPTIONS', 'CONNECT', 'PATH', 'TRACE'];
 ...
 */
function makeSnippet(request: Request, indentation: string, options: SnippetOptions): string {
  ...
  request.body && request.body.mode === 'formdata' && (request.body = solveMultiFile(request.body));

  const body = request.body ?? <RequestBody>{},
